...
def set_info(detec):
    ...
    for (x, y) in detec:
        if (pos_linha + offset) > y > (pos_linha - offset):
            ...
            detec.remove((x, y))
            print("Autos detectados: " + str(carros))                       
           # playsound('C:/Users/Josue/Desktop/CONTADOR_AUTOS/Vehicle-Counter/beep.mp3')


def show_info(frame1, dilatada):
    text = f'Autos: {carros}'
    cv2.putText(frame1, text, (150, 70), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 5)
